...
/* 
 * PHP script to create a excel file incuding orders that must be returned to the seller
 */
ob_start();
session_start();
...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D1', 'office yetişmek zamani');
//query to extract return orders from the db and insert them into the excel report file
$query1 = "select orders.orderID, orders.productPrice, shipment.officeArrivalDate, stat.orderStatus from benneks.orders inner join benneks.shipment on orders.orderID = shipment.orders_orderID " . 
        "inner join benneks.stat on orders.orderID = stat.orders_orderID where stat.orderStatus = 'عودت ترکیه-İade-Turkey'"; 
...
header('Content-Disposition: attachment;filename="iade list Rapor.xls"');
